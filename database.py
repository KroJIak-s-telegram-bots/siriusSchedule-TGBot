...
class dbWorker:
	...
	def pinExists(self, chatId):
		'''Проверка есть ли ID закреплённого сообщения'''
		with self.connection:
			result = self.cursor.execute('SELECT * FROM pins WHERE chatId = ?', (chatId, )).fetchall()
			return bool(len(result))

	def addPin(self, chatId, messageId):
		'''Добавление ID закреплённого сообщения'''
		with self.connection:
			return self.cursor.execute('INSERT INTO pins (chatId, messageId) VALUES(?, ?)', (chatId, messageId))

	def setPin(self, chatId, messageId):
		'''Выставить ID закреплённого сообщения'''
		with self.connection:
			return self.cursor.execute('UPDATE pins SET messageId = ? WHERE chatId = ?', (messageId, chatId))

	def getPin(self, chatId):
		'''Получение ID закреплённого сообщения'''
		with self.connection:
			return self.cursor.execute('SELECT * FROM pins WHERE chatId = ?',(chatId, )).fetchone()[1]